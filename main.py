...
def lambda_handler(event, context):
    if "video_id" in event["params"]["querystring"]:
        ...
        return {"message": "No video id or url provided"}
    ...
    url, log = upload_to_aws(filename, f"{FOLDER_NAME}/{video_info['title']}.mp3")
    os.remove(filename)

    return {"message": log, "download": url}
...
def upload_to_aws(local_file, s3_file):
    ...
    try:
        ...
        url = s3.generate_presigned_url(
            ClientMethod="get_object", Params={"Bucket": BUCKET_NAME, "Key": s3_file}
        )
